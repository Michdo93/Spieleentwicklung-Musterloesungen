...
function setState(state) {
  ...
  const busy = ["Hit", "Kick", "SwordHit"].includes(hero.state) && hero.attackTimer > 0;
  if (busy) return;
  hero.state = state;
  hero.animTime = 0;
}
...
function update(dt) {
  ...
  enemies.forEach((en) => (en.animTime += dt));
...
function render() {
  ...
  ctx.fillText(`Level ${currentLevelNum}   Leben: ${hero.lifeEnergy}/${hero.maxLifeEnergy}   (J Schlag, K Tritt, L Schwert)`, 16, 28);
