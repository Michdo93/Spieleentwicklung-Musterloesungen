 ...
 * Musterloesung
 *
 * Baut auf Kapitel 9 auf. Der Held kann jetzt zuschlagen (J) und treten
 * (K) - mit einer Trefferzone vor ihm, die von der Blickrichtung
 * abhaengt, und einem einzelnen Test-Gegner (noch ohne KI - die kommt
...
// Hit/Kick kommen als neue Animationszustaende dazu - beide NICHT
// endlos (loop:false), genau wie Jump aus Kapitel 5.
...
// Reichweite und Schaden sind bewusst gekoppelt - identisch zu den
// Werten aus DAMAGE in entities.js.
const ATTACKS = {
...
// ein einzelner Test-Gegner, noch ohne KI (siehe Kapitel 13/14)
const enemy = {
  x: 480,
  y: FLOOR_Y - CELL_H,
  hp: 5,
...
function setState(state) {
  ...
  const busy = ["Hit", "Kick"].includes(hero.state) && hero.attackTimer > 0;
  if (busy) return;
  hero.state = state;
  hero.animTime = 0;
}
...
function startAttack(type) {
  if (hero.attackTimer > 0) return; // ein laufender Angriff wird nicht unterbrochen
  hero.state = type;
  hero.animTime = 0;
...
// laufenden Angriffs EINMAL pruefen, ob die Hitbox den Gegner trifft.
...
function render() {
  ...
  ctx.fillText(`Leben: ${hero.lifeEnergy} / 10   (J = Schlag, K = Tritt)`, 16, 28);
}
