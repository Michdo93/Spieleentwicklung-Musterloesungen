...
// entspricht SoundController.as - bündelt Musik und Soundeffekte an
// einem Ort mit gemeinsamer Lautstärke.
...
// entspricht drawHealthBar() in render.js
...
function checkMeleeHit() {
  ...
  for (const en of enemies) {
    ...
    if (overlaps(hitBox, enemyBox)) {
      ...
      if (en.hp === 0) { en.dead = true; points += 100; }
...
// Ereignissen - eine vergessene Aktualisierungsstelle würde sonst zu
// kurzzeitig falschen Anzeigen führen.
function updateHud() {
  document.getElementById("hud-life").textContent = `Leben: ${hero.lifeEnergy}/${hero.maxLifeEnergy}`;
  document.getElementById("hud-points").textContent = `Punkte: ${points}`;
  const parts = [];
  if (hero.hasShuriken) parts.push(`Shuriken: ${hero.shurikenCount}`);
  if (hero.hasSword) parts.push("Schwert");
  document.getElementById("hud-weapons").textContent = parts.join("   ");
  document.getElementById("hud-time").textContent = `Level ${currentLevelNum}`;
}
...
function update(dt) {
  ...
  // entspricht evaluateOutcome(): Held tot -> Niederlage; alle Gegner
  // in Level 2 besiegt -> Sieg. Ein Zeitablauf gibt es in unserem Buch
  // erst mit einem echten Level-Timer (siehe GameManager.timeLeft im
  // fertigen Spiel) - hier vereinfacht auf die beiden Kernbedingungen.
  if (hero.lifeEnergy <= 0) {
    endGame(false, "Der Held ist gestorben");
  } else if (currentLevelNum === 2 && enemies.every((en) => en.dead)) {
    endGame(true, "Beide Level ohne Tod abgeschlossen!");
  }
}
...
function render() {
  ...
  enemies.forEach((en) => {
    ...
    drawHealthBar(en.x, en.y - 90, en.hp, en.maxHp);
  });
...
// und setzt es nur auf den gewünschten.
...
// entspricht dem Highscore-Teil von ui.js: localStorage statt eines
// Servers - GitHub Pages kann keinen eigenen Server betreiben, braucht
// dafür aber auch keinen.
const HIGHSCORE_KEY = "ninjafight-ch18-highscores";

function loadHighscoreList() {
  try { return JSON.parse(localStorage.getItem(HIGHSCORE_KEY) || "[]"); }
  catch (e) { return []; }
}
function saveHighscoreEntry(name, points, level) {
...
// entspricht evaluateOutcome() bzw. winGame()/endGame() in GameManager
function endGame(won, reason) {
  showScreen("end");
  document.getElementById("end-title").textContent = won ? "Sieg!" : "Game Over";
  document.getElementById("end-points").textContent = `${reason} - Punkte: ${points}`;
  renderHighscoreList();
}

document.getElementById("btn-send-score").addEventListener("click", () => {
  const name = document.getElementById("score-name").value;
  saveHighscoreEntry(name, points, currentLevelNum);
  renderHighscoreList();
});
...
// entspricht nextLevel()/restartGame() im vollständigen Spiel - hier
// vereinfacht auf unsere zwei Test-Level.
function resetGame() {
  currentLevelNum = 1;
  level = buildLevel(1);
  enemies = spawnEnemies(1);
  ...
  points = 0;
  gameTime = 0;
  hero.x = 60; hero.y = 414.1; hero.vy = 0; hero.facing = 1;
  hero.state = "Idle"; hero.animTime = 0; hero.onGround = true; hero.onLadder = false;
  hero.lifeEnergy = hero.maxLifeEnergy; hero.invulnTimer = 0;
  hero.attackTimer = 0; hero.attackHitDone = false;
}
