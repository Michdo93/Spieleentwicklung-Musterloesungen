...
function update(dt) {
  ...
  // entspricht onHeroDeath(): keine Lebensenergie mehr -> Spiel vorbei
...
// entspricht Highscore-Teil von ui.js: localStorage statt eines
// PHP-Servers - GitHub Pages kann keinen eigenen Server betreiben,
// braucht dafuer aber auch keinen.
...
function renderHighscoreList() {
  ...
  el.textContent = list.length === 0
    ...
    : list.map((e, i) => `${i + 1}. ${e.name} - ${e.points}`).join("\n");
}

// entspricht endGame() in gamemanager.js
function endGame() {
  showScreen("gameover");
  document.getElementById("go-points").textContent = `Punkte: ${points}`;
  renderHighscoreList();
}
...
// entspricht dem, was im vollstaendigen Spiel nextLevel()/restartGame()
// uebernehmen wuerde - hier vereinfacht auf einen einzelnen Neustart
// unseres einen Test-Levels.
function resetGame() {
  hero.x = 150; hero.y = FLOOR_Y - CELL_H; hero.vy = 0; hero.facing = 1;
  ...
  enemy.x = 40; enemy.y = FLOOR_Y - CELL_H; enemy.hp = enemy.maxHp; enemy.dead = false;
  projectiles = [];
  points = 0;
  gameTime = 0;
}
